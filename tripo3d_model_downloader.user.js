// ==UserScript==
// @name         Tripo3D Model Downloader
// @namespace    https://github.com/local/tripo3d-model-downloader
// @version      2.0.0
// @description  Extracts the rendered model straight out of the Three.js scene on studio.tripo3d.ai and downloads it as GLB or OBJ+MTL+texture.
// @author       local
...
(function () {
  ...
  /**
   * Strategy (c): Three.js keeps a reference from the WebGL context back to the
   * renderer on most builds; failing that, some apps stash the scene on the
   * canvas element itself. Brute-force every canvas on the page.
   */
  function findSceneViaCanvas() {
    for (const canvas of document.querySelectorAll('canvas')) {
      ...
      // __threeObj / __r3f style back-references
      const r = canvas.__three_renderer || canvas.__renderer;
      const s = unref(r?.scene) || unref(r?._scene);
  ...
  function findScene() {
    ...
    throw new Error(
      'THREE.Scene not found. Make sure the model is fully rendered in the viewport, ' +
      'then re-run. If the page was just navigated to, reload it first.'
    );
  }
  ...
  window.tripo = { glb: exportGLB, obj: exportOBJ, inspect, panel: mountPanel, findScene };
  ...
  log('ready — use the panel, or call tripo.glb() / tripo.obj() / tripo.inspect()');
})();
...
(function bootstrap() {
  ...
  scheduleSync();
  console.log('[tripo] userscript v2.0.0 active on', location.hostname);
})();
